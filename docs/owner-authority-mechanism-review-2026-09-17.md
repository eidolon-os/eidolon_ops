...
`pi5-owner-generation-divergence-2026-09-16.md`（当前在分支 `claude/epic-jackson-d70852`）。
...
用户数据的收敛动词。** 今天缺的那个动词大约是 `authority resync --from-host`：按板子
已证明的值重写控制器缓存，打印将要改什么，任何密钥材料不一致就拒绝。

值得注意的是：做了 A 之后这个动词就不需要存在了（没有东西可同步）。这本身就是选 A
的理由。
...
- 但方案 C 之后还有第二道闸门（`bind_delivery(allow_create=False)`，而 pi5 没有
  `host_delivery.json`）。这道闸门在方案 B 里会变成第一道，而且它需要的不是"绕过"，
  而是**补录一条交付绑定**——这块板子的交付真实发生过（2026-08-27），只是那时还没有
  这个文件。所以 B 里应该有一个明确的、只在"板子已持有本 Owner 的授权且自洽"时允许的
  `adopt` 语义，而不是靠 `allow_create` 这个布尔开关。
